...
Use the versioned registry and lifecycle envelope schemas by reference. Check the assigned active task only; do not select/create tasks, mutate status, finish/archive, commit/push, publish tracker state, write global knowledge, or recursively dispatch. Return blockers to `execute.implement`, otherwise `finish.spec-update`.
