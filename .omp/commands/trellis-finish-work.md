...
Before archive, run the lifecycle completion gate for the active task. It must pass the canonical `evidence/check.json`, conditional `evidence/review.json`, `evidence/spec-disposition.json`, and `evidence/commit-readiness.json` contracts. Do not archive or clear task state while a blocker remains; return to the gate's named lifecycle seam instead.
