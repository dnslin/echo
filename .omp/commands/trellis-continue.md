...
For specialist work, route by the category table in `.trellis/workflow.md`, then resolve the exact registry entry. Resume at the registry entry's `return_points`; never let a specialist select a different task or mutate lifecycle state.
