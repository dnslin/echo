...
## After Analysis: Return To Trellis

When this skill is invoked inside an active Trellis task, the lifecycle invocation envelope overrides any conflicting publication behavior:

1. Write the analysis to the exact task-local target supplied by the envelope, normally `<task>/research/diagnosis/<bug>.md`.
2. Return proposed spec/guide changes as `knowledge_candidates`; do not edit repository-global specs or guides from this nested skill.
3. Return follow-up work as blockers or lifecycle recommendations; do not create tracker issues, select/create another task, dispatch implement/check, or mutate task status.
4. Do not sync templates, commit, archive, finish, push, or publish. Phase 3.3 owns approved spec application and Phase 3.4 owns commits.

Outside a Trellis invocation, follow the host workflow's ownership rules rather than assuming this skill owns publication.
