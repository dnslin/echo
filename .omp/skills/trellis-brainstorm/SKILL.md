...
## Specialist Routing Boundary

When feature shaping, intake/triage, architecture, domain modeling, research, prototyping, diagnosis, decomposition, UI, Python, SDK, or release expertise is needed, resolve the specialist through the bundled registry and pass a versioned `trellis.skill-invocation` envelope. Do not reproduce the specialist's full instructions here. The envelope's task path, phase, artifact target, side effects, and return seam override conflicting tracker publication, global-write, task-selection, commit/archive/status, or recursive-dispatch clauses. Merge the returned `trellis.skill-result` into the owning planning artifacts.
