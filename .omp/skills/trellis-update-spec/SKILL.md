...
## Trellis Lifecycle Output

This skill may apply repository-global code-spec changes only when invoked by outer workflow Phase 3.3. Nested specialists instead return `knowledge_candidates` and must not call this skill recursively.

After deciding, write `<task>/evidence/spec-disposition.json` using schema `trellis.spec-disposition` from `.trellis/scripts/common/lifecycle_contract.py`: `updated` with existing artifact paths, `not_needed` with a rationale, or `deferred` with an explicit blocker. This skill does not select tasks, mutate status, commit, archive, push, or publish tracker state.
