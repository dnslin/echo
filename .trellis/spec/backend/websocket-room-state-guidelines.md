...
- Router code must redact the `token` query parameter from Gin context, recovery, and logging surfaces while passing the original request or extracted token to authentication logic.
- Accepted connections immediately receive `room.snapshot` before they are exposed to later room broadcasts.
...
- Good: WebSocket handshake verifies the query token, checks path-room match, authorizes persisted product room/member state, upgrades, queues the initial snapshot, registers the connection, then starts read/write/heartbeat loops.
...
- Good: `POST /v1/rooms/{room_id}/leave` remains the only product leave mutation path; after success, the handler notifies the hub to broadcast `member.left` and close the leaving connection.
- Base: snapshots are projections rebuilt from SQLite room/member state plus the receiving member ID; they are not persisted as separate state.
- Base: room sequence counters, WebSocket connections, heartbeat pings, and send queues are in-memory single-instance transport state.
...
- Bad: registering a connection for room broadcasts before its initial snapshot has been queued.
- Bad: logging raw WebSocket URLs that contain `?token=...`.
...
- HTTP/router tests:
  ...
  - redacted request preserves non-sensitive query fields, leaves the original request unchanged, and still passes the original token to the hub.
- Store tests:
